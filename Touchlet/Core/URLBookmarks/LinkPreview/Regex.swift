...
class Regex {
    ...
    static func pregMatchFirst(_ string: String, regex: String, index: Int = 0) -> String? {
        do {
            ...
            guard let match = rx.firstMatch(in: string, options: [], range: NSRange(location: 0, length: string.count)) else{
                return nil
            }
    ...
    private static func stringMatches(_ results: [NSTextCheckingResult], text: String, index: Int = 0) -> [String] {
        return results.map {
    ...
    // Return tag pattern
    static func tagPattern(_ tag: String) -> String {return "<" + tag + "(.*?)>(.*?)</" + tag + ">"}
}
